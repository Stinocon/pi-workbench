...
- Enforced offload rules (harness-enforced, NOT advisory): \`~/.pi/agent/offload-rules.json\`. After modifying a critical file, an adversarial review by a DIFFERENT model (\`subagent(agent="miner-xhigh", …)\`) must run before the task is done — the harness blocks the "done" claim otherwise. Comply proactively; do not self-review with the same model.
